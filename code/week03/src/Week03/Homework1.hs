...
mkValidator (VestingDatum b1  b2  deadline) _ ctx =
      traceIfFalse "First beneficiary sig is missing" (checkSig b1)           &&
      traceIfFalse "Current slot is after the deadline" beforeDealine         ||
      traceIfFalse "Second beneficiary sig is missing" (checkSig b2)          &&
      traceIfFalse "Current slot is before the deadline" (not beforeDealine)
    where
        info :: TxInfo
        info =  scriptContextTxInfo ctx
        checkSig :: PubKeyHash -> Bool
        checkSig pk = pk `elem` txInfoSignatories info
        beforeDealine :: Bool
        beforeDealine = to deadline `contains` txInfoValidRange info
